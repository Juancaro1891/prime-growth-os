...
- `NEXT_PUBLIC_SUPABASE_URL` / `SUPABASE_URL` — Supabase REST endpoint, shared by the `leads`, `meta_accounts`, and `business_analysis` tables.
...
- `SUPABASE_SERVICE_ROLE_KEY` — Supabase service-role key, read via `lib/supabase-admin.ts` and used **only** server-side (never exported to the client) by `lib/meta.ts` (`meta_accounts`) and `lib/business-analysis.ts` (`business_analysis`). Both tables hold sensitive data (live Meta access tokens with `ads_management` scope; AI-generated business analysis) deliberately kept off the anon key + RLS-less path the `leads` table uses — both have RLS enabled with no public policies, so only the service-role key (which bypasses RLS) can touch them.
